...
#FP2
fP_2_array=[[0]]*nFus#i need just 1 point for each segment which corresponds to eta=0,xsi=0.5
#to extract the upper or lower points (is a CPACS characteristic)
for i in range(0,nFus):
    for j in range(0,fusSegCount[i]):
...
#FP3 
fP_3_array=[[0]]*nFus#i need just 1 point for each segment which corresponds to eta=0,xsi=0.25
#to extract the upper or lower points (is a CPACS characteristic)
for i in range(0,nFus):
    for j in range(0,fusSegCount[i]):
...
#FP4
fP_4_array=[[0]]*nFus#i need just 1 point for each segment which corresponds to eta=0,xsi=0.75
#to extract the upper or lower points (is a CPACS characteristic)
for i in range(0,nFus):
    for j in range(0,fusSegCount[i]):
...
for i in range(0,nFus):
    for j in range(0,fusSegCount[i]+1):
        if math.fabs(fP_up_2[i][j])-math.fabs(fP_down_2[i][j])<10**(-5):
            if math.fabs(fP_up[i][j])!=0 and math.fabs(fP_up[i][j])!=0:     #for FRP==XY CPACS plane
                section_diameter[i][j]=math.fabs(fP_up[i][j]-fP_down[i][j])
            else:
                section_diameter[i][j]=math.fabs(fP_up_2[i][j]-fP_down_2[i][j]) 
        else :
            section_diameter[i][j]=math.fabs(fP_up_2[i][j]-fP_down_2[i][j]) 
print "diametri_z = {}".format(section_diameter)
...
#FP1
#i need just 1 point for each segment which corresponds to eta=0,xsi=1
...
#FP2
#i need just 1 point for each segment which corresponds to eta=0,xsi=0.5
#to extract the right or left points (is a CPACS characteristic)
fP_2_y=[[0]]*nFus  
for i in range(0,nFus):
    for j in range(0,fusSegCount[i]):
...
#FP3 
#i need just 1 point for each segment which corresponds to eta=0,xsi=0.25
#to extract the right or left points (is a CPACS characteristic)
fP_3_y=[[0]]*nFus  
for i in range(0,nFus):
    for j in range(0,fusSegCount[i]):
...
#FP4
#i need just 1 point for each segment which corresponds to eta=0,xsi=0.75
#to extract the right or left points (is a CPACS characteristic)
fP_4_y=[[0]]*nFus  
for i in range(0,nFus):
    for j in range(0,fusSegCount[i]):
...
#diff_y and diff_2_y -->vectors of the difference between fP_up and fP_left
#iniYialization
diff_y=[[0]]*nFus
for i in range(0,nFus):
    for j in range(0,fusSegCount[i]+1):
        diff_y=[[0]*(fusSegCount[i]+1)]
for i in range(0,nFus):
    for j in range(0,fusSegCount[i]+1):
        diff_y[i][j]=math.fabs(fP_right[i][j]-fP_left[i][j])
diff_2_y=[[0]]*nFus
for i in range(0,nFus):
    for j in range(0,fusSegCount[i]+1):
        diff_2_y=[[0]*(fusSegCount[i]+1)]
for i in range(0,nFus):
    for j in range(0,fusSegCount[i]+1):
        diff_2_y[i][j]=math.fabs(fP_right_2[i][j]-fP_left_2[i][j])
...
for i in range(0,nFus):
    for j in range(0,fusSegCount[i]+1):
        if math.fabs(fP_right_2[i][j])-math.fabs(fP_left_2[i][j])<10**(-5):
            if math.fabs(fP_right[i][j])!=0 and math.fabs(fP_left[i][j])!=0:     #for FRP==XZ CPACS plane
                section_diameter_y[i][j]=math.fabs(fP_right[i][j]-fP_left[i][j])
            else:
                section_diameter_y[i][j]=math.fabs(fP_right_2[i][j]-fP_left_2[i][j]) 
        else:
            section_diameter_y[i][j]=math.fabs(fP_right_2[i][j]-fP_left_2[i][j])
print "diametri_y = {}".format(section_diameter_y)
...
## Tailcone and nose
if np.max(diff_2)<10**(-2):
    epsilon=10**(-5)#reference axes-->fuselage lower side ->z coordinates cabin =0 but the beginning cone section z=4*10^-5, we need epsilon
    j=-1 
    while j>(-len(x_ell[FusPos])): #from the end
        if math.fabs(fP_down[FusPos][j]-fP_down[FusPos][j-1])<epsilon:
            in_cone=fusSecCount[FusPos]+j
            print "Tailcone starts at section number = {}".format(in_cone)#ok
            break
        j=j-1
    for j in range(0,len(x_ell[FusPos])):
        if math.fabs(fP_down[FusPos][j+1]-fP_down[FusPos][j])<epsilon:
...
if np.max(diff)<10**(-2):
    ...
    while j>(-len(x_ell[FusPos])):
        if math.fabs(fP_down_2[FusPos][j]-fP_down_2[FusPos][j-1])<epsilon:
            in_cone=fusSecCount[FusPos]+j+1
            print "Tailcone starts at section number = {}".format(in_cone)
            break
        j=j-1
    for j in range(0,len(x_ell[FusPos])):
        if math.fabs(fP_down_2[FusPos][j+1]-fP_down_2[FusPos][j])<epsilon:
            print "Nose finishes at section number = {}".format(j)
            in_nose=j
            break
...
print 'Root chord of main wing = {}'.format(Croot_w)
...
if WettedParameter==0:
    ...
    if aircraftConfiguration==0:
        ...
        if np.max(diff)<10**(-2):
            disp_ref=d_fus_max_z_array[0]/2-fP_up_2[FusPos][in_cone]-Z 
        print "Translation of FPR respects to CPACS XY-plane = {}".format(disp_ref)
        
        Z_wing=Z_j_LE[mainWPos]+disp_ref  #in fuselage plane reference
...
if aircraftConfiguration==0:
    ...
    print "Wing Z-position in FPR= {}".format(Z_wing) # Leading Edge point of wing section in fuselage 
    j=0
    while j<(len(x_ell[FusPos])):
        if x_ell[FusPos][j+1]>X_inner[mainWPos][0] and x_ell[FusPos][j]<X_inner[mainWPos][0]:
            diameter_WingFuselage=(section_diameter[FusPos][j]+section_diameter[FusPos][j+1])/2  #APPROXIMATION: maximum diameter in Z-direction and linear interpolation
    ...
    if Z_wing/(diameter_WingFuselage/2)<-0.5:
        ...
        h_wing=0.5
        print "Mid wing aircraft configuration"
    CD0_int_wf = ((0.5*h_wing**2+1.25*h_wing+0.75)*2.16*Croot_w**2*(max_thickness_array[mainWPos][0])**3)/Sw
    ...
    while j>(-len(x_ell[FusPos])):
        if x_ell[FusPos][j]>Lf-0.25*Lcone and x_ell[FusPos][j-1]<Lf-0.25*Lcone:
            if np.max(diff_2)<10**(-2):
                ...
                z_tail=(fP_up_2[FusPos][j]+Z+fP_up_2[FusPos][j-1]+Z)/2
                diameter_upsweep=(section_diameter[FusPos][j]+section_diameter[FusPos][j-1])/2
