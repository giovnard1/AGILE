...
# This file is automatically created from tigl.h on 2016-02-04.
...
def catch_error(returncode, *args, **kwargs):
    if returncode != TiglReturnCode.TIGL_SUCCESS:
        raise TiglException(returncode, args, kwargs)
...
class Tigl(object):
    ...
    try:
        if sys.platform == 'win32':
            lib = ctypes.cdll.TIGL
        elif sys.platform == 'darwin':
            lib = ctypes.CDLL("libTIGL.dylib")
        else:
            lib = ctypes.CDLL("libTIGL.so")
    except:
        raise Exception("Could not load the TIGL library. Please check if:\n" +
    ...
    def wingComponentSegmentPointGetSegmentEtaXsi(self, componentSegmentUID, eta, xsi):
        ...
        # call to native function
        errorCode = self.lib.tiglWingComponentSegmentPointGetSegmentEtaXsi(self._handle, _c_componentSegmentUID, _c_eta, _c_xsi, ctypes.byref(_c_wingUID), ctypes.byref(_c_segmentUID), ctypes.byref(_c_segmentEta), ctypes.byref(_c_segmentXsi), ctypes.byref(_c_errorDistance))
        ...
        return (_py_wingUID, _py_segmentUID, _py_segmentEta, _py_segmentXsi, _py_errorDistance)
    ...
    def componentIntersectionPoint(self, componentUidOne, componentUidTwo, lineID, eta):
        # input arg conversion
        ...
        # call to native function
        errorCode = self.lib.tiglComponentIntersectionPoint(self._handle, _c_componentUidOne, _c_componentUidTwo, _c_lineID, _c_eta, ctypes.byref(_c_pointXPtr), ctypes.byref(_c_pointYPtr), ctypes.byref(_c_pointZPtr))
        catch_error(errorCode, 'tiglComponentIntersectionPoint', componentUidOne, componentUidTwo, lineID, eta)
    ...
    def componentIntersectionPoints(self, componentUidOne, componentUidTwo, lineID, etaArray, numberOfPoints):
        # input arg conversion
        _c_componentUidOne = ctypes.c_char_p(encode_for_c(componentUidOne))
        _c_componentUidTwo = ctypes.c_char_p(encode_for_c(componentUidTwo))
        _c_lineID = ctypes.c_int(lineID)
        array_t_etaArray = ctypes.c_double * len(etaArray)
        _c_etaArray = array_t_etaArray(*etaArray)
        _c_numberOfPoints = ctypes.c_int(numberOfPoints)

        # output arg preparation
        pointXArray_len = 1 * numberOfPoints
        _c_pointXArray = (ctypes.c_double * pointXArray_len)()
        pointYArray_len = 1 * numberOfPoints
        _c_pointYArray = (ctypes.c_double * pointYArray_len)()
        pointZArray_len = 1 * numberOfPoints
        _c_pointZArray = (ctypes.c_double * pointZArray_len)()

        # call to native function
        errorCode = self.lib.tiglComponentIntersectionPoints(self._handle, _c_componentUidOne, _c_componentUidTwo, _c_lineID, _c_etaArray, _c_numberOfPoints, ctypes.byref(_c_pointXArray), ctypes.byref(_c_pointYArray), ctypes.byref(_c_pointZArray))
        catch_error(errorCode, 'tiglComponentIntersectionPoints', componentUidOne, componentUidTwo, lineID, etaArray, numberOfPoints)

        pointXArray_array_size = pointXArray_len
        _py_pointXArray = tuple(_c_pointXArray[i] for i in range(pointXArray_array_size))
        pointYArray_array_size = pointYArray_len
        _py_pointYArray = tuple(_c_pointYArray[i] for i in range(pointYArray_array_size))
        pointZArray_array_size = pointZArray_len
        _py_pointZArray = tuple(_c_pointZArray[i] for i in range(pointZArray_array_size))

        return (_py_pointXArray, _py_pointYArray, _py_pointZArray)



    def componentIntersectionLineCount(self, componentUidOne, componentUidTwo):
        # input arg conversion
        _c_componentUidOne = ctypes.c_char_p(encode_for_c(componentUidOne))
        _c_componentUidTwo = ctypes.c_char_p(encode_for_c(componentUidTwo))

        # output arg preparation
        _c_numWires = ctypes.c_int()

        # call to native function
        errorCode = self.lib.tiglComponentIntersectionLineCount(self._handle, _c_componentUidOne, _c_componentUidTwo, ctypes.byref(_c_numWires))
        catch_error(errorCode, 'tiglComponentIntersectionLineCount', componentUidOne, componentUidTwo)

        _py_numWires = _c_numWires.value
    ...
    def exportMeshedFuselageVTKSimpleByUID(self, fuselageUID, filenamePtr, deflection):
        # input arg conversion
        _c_fuselageUID = ctypes.c_char_p(encode_for_c(fuselageUID))
        _c_filenamePtr = ctypes.c_char_p(encode_for_c(filenamePtr))
        _c_deflection = ctypes.c_double(deflection)
        ...
        # call to native function
        errorCode = self.lib.tiglExportMeshedFuselageVTKSimpleByUID(self._handle, _c_fuselageUID, _c_filenamePtr, _c_deflection)
        catch_error(errorCode, 'tiglExportMeshedFuselageVTKSimpleByUID', fuselageUID, filenamePtr, deflection)
